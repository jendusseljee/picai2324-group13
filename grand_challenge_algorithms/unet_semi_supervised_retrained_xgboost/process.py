...
class csPCaAlgorithm(SegmentationAlgorithm):
    ...
    def __init__(self):
        ...
        self.xgb_model = xgb.XGBClassifier(max_depth=3, max_leaves=10, n_estimators=50, learning_rate=0.12, objective='binary:logistic', eval_metric='auc', grow_policy='lossguide', tree_method='exact', booster='dart', scale_pos_weight=0.28, gamma=0.9)

        self.xgb_model.load_model(self.algorithm_weights_dir / 'xgboost.model')
    ...
    def predict(self):
        ...
        features = ['patient_age', 'psa', 'psad', 'prostate_volume']
        data = [[self.age, self.psa, self.psad, self.prostate_volume]]
        ...
        for i in range(5):
            df[f'f{i}_max'] = [np.max(outputs[i]).astype('float32')]
