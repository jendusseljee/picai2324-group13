...
class csPCaAlgorithm(SegmentationAlgorithm):
    ...
    def predict(self):
        ...
        # Add tabular data
        tabular = np.stack([np.array([self.age, self.psa, self.psad, self.prostate_volume], dtype='float32')])
        ...
        for p in range(len(self.models)):
            ...
            with torch.no_grad():
                ...
                preds = [
                    torch.sigmoid(self.models[p](x, tabular))[:, 1, ...].detach().cpu().numpy()
