...
class VNet(nn.Module):
    ...
    def __init__(
        ...
        self.in_tr = InputTransition(spatial_dims, in_channels, 24, act, bias=bias)
        self.down_tr32 = DownTransition(spatial_dims, 24, 1, act, bias=bias)
        self.down_tr64 = DownTransition(spatial_dims, 48, 2, act, bias=bias)
        self.down_tr128 = DownTransition(spatial_dims, 96, 3, act, dropout_prob=dropout_prob_down, bias=bias)
        self.down_tr256 = DownTransition(spatial_dims, 192, 2, act, dropout_prob=dropout_prob_down, bias=bias)
        self.up_tr256 = UpTransition(spatial_dims, 384, 384, 2, act, dropout_prob=dropout_prob_up)
        self.up_tr128 = UpTransition(spatial_dims, 384, 192, 2, act, dropout_prob=dropout_prob_up)
        self.up_tr64 = UpTransition(spatial_dims, 192, 96, 1, act)
        self.up_tr32 = UpTransition(spatial_dims, 96, 48, 1, act)
        self.out_tr = OutputTransition(spatial_dims, 48, out_channels, act, bias=bias)

    def forward(self, x):
        x = self.upsample_pad(x)
        out16 = self.in_tr(x)
        ...
        x = self.out_tr(x)
        x = self.downsample_pad(x)
        return x
    
    def upsample(self, x):
        return nn.functional.interpolate(x, mode="nearest", scale_factor=(32/20,1,1))
    
    def downsample(self, x):
        return nn.functional.interpolate(x, mode="nearest", scale_factor=(20/32,1,1))

    def upsample_pad(self, x):
        return nn.functional.pad(x,(0,0,0,0,6,6))
    
    def downsample_pad(self, x):
        return torch.narrow(x,2,6,20)